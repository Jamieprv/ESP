...
if not syn and not PROTOSMASHER_LOADED then print'This ESP only officially supports Synapse and Protosmasher!' end
...
local LastRefresh = 0
local OptionsFile = 'Jamie_ESP_SETTINGS.dat'
local Binding = false
local BindedKey = nil
local OIndex = 0
local LineBox = {}
...
shared.RSName = shared.RSName or ('ESP_by_Jamie-' .. HttpService:GenerateGUID(false))
...
Options('TextSize', 'Text Size', syn and 24 or 18, 14, 28); -- cuz synapse fonts look weird???
Options('MaxDistance', 'Max Distance', 2500, 100, 25000);
Options('RefreshRate', 'Refresh Rate (ms)', 5, 1, 200);
Options('YOffset', 'Y Offset', 0, -200, 200);
Options('MenuKey', 'Gui Hide/Show', Enum.KeyCode.F4, 1);
Options('ToggleKey', 'Enable/Disable', Enum.KeyCode.F3, 1);
Options('ChangeColors', SENTINEL_LOADED and 'Sentinel Unsupported' or 'Change Colors', function()
	...
	SubMenu:Show(GetMouseLocation(), 'ESP Colors', {
		{
...
function CreateMenu(NewPosition) -- Create Menu
	...
	Menu:AddMenuInstance('TopBarTextBR', 'Text', {
		Size 		= 18;
		Position	= shared.MenuDrawingData.Instances.TopBarTwo.Position + V2New(BaseSize.X - 95, 15);
		Text		= 'by Jamie';
		Color		= Colors.Secondary.Light;
		Visible		= true;
		Transparency= 1;
	...
	GetTableData(Options)(function(i, v) -- just to make sure certain things are drawn  or after others, too lazy to actually sort table
